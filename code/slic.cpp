...
void Slic::colour_with_cluster_means(IplImage *image) {
	vector<CvScalar> colours(centers.size(), 0);
	
	//cout << cvGet2D(image, 0, 0).val[0] << endl;
	...
	//		std::cout << colours[index].val[0] << std::endl;
	...
	for (int i = 0; i < image->width; i++) {
		for (int j = 0; j < image->height; j++) {
			...
			//cout << ncolour.val[0] << " " << ncolour.val[1] << " " << ncolour.val[2] << endl;
